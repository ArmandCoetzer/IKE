...
public class PurchaseOrdersControllerClientPOTests
{
    ...
    {
        contentRoot ??= Path.Combine(Path.GetTempPath(), "IkeTests_" + Guid.NewGuid().ToString("N"));
    ...
    {
        var (db, poId) = CreateDbWithOnePO();
        var tempDir = Path.Combine(Path.GetTempPath(), "IkeTests_" + Guid.NewGuid().ToString("N"));
        try
        {
            var mockFile = new Mock<IFormFile>();
            mockFile.Setup(f => f.Length).Returns(100);
