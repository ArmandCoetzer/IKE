...
public static class WaAmendmentSignOffHelper
{
    ...
    public static async Task<bool> JobHasExpiredWorkAuthorisationStandstillAsync(ApplicationDbContext db, Guid jobCardId, CancellationToken ct)
    {
        var job = await db.JobCards.AsNoTracking()
            ...
            .Select(j => new { j.PermitsRequired, j.PaperPermitMode, j.Status })
            .FirstOrDefaultAsync(ct);
        if (job == null)
            return false;
        if (!job.PermitsRequired || job.PaperPermitMode || JobCardStatus.IsCompletedLike(job.Status))
            return false;
        var todaySa = IkeTimeHelper.TodayInSouthAfrica(DateTime.UtcNow);
        ...
        var hasExpiredWa = permits.Any(p =>
            ...
            || (p.ValidTo.HasValue && IkeTimeHelper.TodayInSouthAfrica(p.ValidTo.Value) < todaySa));
        if (!hasExpiredWa) return false;

        var hasSignedValidWa = permits.Any(p =>
            !PermitStatus.IsExpiredLike(p.Status)
            && (p.ValidTo == null || IkeTimeHelper.TodayInSouthAfrica(p.ValidTo.Value) >= todaySa)
