...
public class JobCardsController : ControllerBase
{
    ...
    public async Task<ActionResult<JobCardListDto>> Update(Guid id, [FromBody] UpdateJobCardRequest request, CancellationToken ct = default)
    {
        ...
        if (request.PermitsRequired.HasValue)
        {
            ...
            if (!request.PermitsRequired.Value)
            {
                j.ActiveJobPermitId = null;
                j.PaperPermitMode = false;
            }
        }
        ...
        {
            var allowedPartCompanyIds = new HashSet<Guid>();
            if (j.Site?.CompanyId is Guid siteCompanyId)
                allowedPartCompanyIds.Add(siteCompanyId);
            if (j.Site?.Company?.ParentCompanyId is Guid parentCompanyId)
                allowedPartCompanyIds.Add(parentCompanyId);
            ...
            foreach (var pp in request.PlannedParts)
            {
                ...
                if (allowedPartCompanyIds.Count == 0) continue;
                var partInScope = await _db.Parts.AnyAsync(p =>
                    p.Id == pp.PartId
                    && p.CompanyId.HasValue
                    && allowedPartCompanyIds.Contains(p.CompanyId.Value), ct);
