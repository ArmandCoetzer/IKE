...
public class AuthController : ControllerBase
{
    ...
    public async Task<ActionResult<AuthResponse>> LoginWeb([FromBody] LoginRequest request, CancellationToken ct = default)
    {
        ...
        {
            await LogFailedLoginAttemptAsync("web", request, result.ErrorMessage ?? "Invalid credentials.", ct);
        ...
        {
            await LogFailedLoginAttemptAsync("web", request, "Technician account attempted web login.", ct);
    ...
    public async Task<ActionResult<AuthResponse>> LoginMobile([FromBody] LoginRequest request, CancellationToken ct = default)
    {
        ...
        if (result.User == null)
        {
            await LogFailedLoginAttemptAsync("mobile", request, result.ErrorMessage ?? "Invalid credentials.", ct);
            return Unauthorized(new { message = result.ErrorMessage ?? "Invalid email or password." });
        }
        return Ok(result.Response);
    }

    private async Task LogFailedLoginAttemptAsync(string channel, LoginRequest request, string reason, CancellationToken ct)
    {
        var maskedEmail = MaskEmail(request.Email);
        _logger.LogWarning(
            "Failed {Channel} login attempt for {Email}: {Reason}",
            channel,
            maskedEmail,
            reason);
        ...
        try
        {
            _db.AuditLogs.Add(new AuditLog
            {
                ...
                Details = $"channel={channel}; email={maskedEmail}; reason={reason}"
            });
            await _db.SaveChangesAsync(ct);
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "Failed to record failed-login audit entry.");
        }
    }
    ...
    private static string MaskEmail(string? email)
    {
        ...
        if (at <= 0)
            return string.IsNullOrWhiteSpace(trimmed) ? "(blank)" : "***";
...
""";
        ...
        {
            await _db.SaveChangesAsync(ct);
            var subject = "Reset your IKE password";
