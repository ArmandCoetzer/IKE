# Permit field catalog (IKE field service)

This document lists fields for **Work Authorisation (WA)** and each **child permit** type.  
...
- Child checklists + structured forms: `Permits/IkePermitTemplateCatalog.cs`, `Permits/IkePermitFormFieldSets.cs`.
...
2. **Structured form** — `FormSchemaJson` / `formFields` + `formValues` (see `IkePermitFormFieldSets` for ids).
