...
using Tradion.Api.DTOs.JobCardWork;
...
public class JobCardWorkController : ControllerBase
{
    ...
    public async Task<ActionResult<JobCardWorkDto>> Get(Guid id, CancellationToken ct = default)
    {
        ...
        var linkedPos = await _db.PurchaseOrders.AsNoTracking()
            ...
            .ToListAsync(ct);
        var visiblePermitsOrdered = PaperPermitModeHelper.VisiblePermits(job.Permits).OrderByDescending(p => p.PermitNumber).ToList();
        ...
        var dto = new JobCardWorkDto
        {
            ...
            CompletedAt = await _db.AuditLogs.AsNoTracking()
                ...
                .Select(a => (DateTime?)a.CreatedAt)
                .FirstOrDefaultAsync(ct),
            FirstPermitRequestedAt = visiblePermitsOrdered.OrderBy(p => p.RequestedAt).Select(p => (DateTime?)p.RequestedAt).FirstOrDefault(),
            FirstPermitApprovedAt = visiblePermitsOrdered.Where(p => p.ApprovedAt.HasValue).OrderBy(p => p.ApprovedAt).Select(p => p.ApprovedAt).FirstOrDefault(),
            ...
            ActivePermitId = job.ActiveJobPermitId.HasValue && job.Permits.Any(p => p.Id == job.ActiveJobPermitId.Value && !p.HiddenFromUiForHistory)
                ? job.ActiveJobPermitId
                : null,
            ActivePermitName = job.ActiveJobPermitId.HasValue && job.Permits.Any(p => p.Id == job.ActiveJobPermitId.Value && !p.HiddenFromUiForHistory)
                ? PermitTemplateDurationHelper.PrimaryDisplayName(job.ActiveJobPermit?.PermitTemplate)
                : null,
            BlockedReason = job.BlockedReason,
            PendingWaAmendmentSignOff = visiblePermitsOrdered.Any(p => p.PendingWaAmendmentSignOff),
            WaExpiredStandstill = waExpiredStandstill,
            PaperPermitMode = job.PaperPermitMode,
            CanActivatePaperPermitMode = PaperPermitModeHelper.CanActivatePaperPermitMode(job)
