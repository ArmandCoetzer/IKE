    ...
    public async Task<IActionResult> Send(Guid id, [FromQuery] string? toEmail, [FromQuery] bool attachPdf = true, CancellationToken ct = default)
    {
        ...
        if (!sent)
            return BadRequest(ApiResponseBodies.Message(
                "The quote email was not sent. Common causes: missing client contact email (or toEmail query), missing or wrong SMTP password in SmtpSettings, or the SMTP server rejected the message — check API logs."));
