...
public static class PermitRolloverHelper
{
    public static async Task EnsureLazyRolloverOnReadAsync(ApplicationDbContext db, Guid jobCardId, string? actingUserId, CancellationToken ct)
    {
        ...
        var todaySa = TradionTimeHelper.TodayInSouthAfrica(utcNow);
        var changed = false;
    ...
    private static DateTime SaDate(DateTime utcLike) => TradionTimeHelper.TodayInSouthAfrica(utcLike.Kind == DateTimeKind.Utc ? utcLike : DateTime.SpecifyKind(utcLike, DateTimeKind.Utc));
