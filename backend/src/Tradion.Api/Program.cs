...
if (builder.Environment.IsEnvironment("Testing"))
{
    builder.Services.AddDbContext<ApplicationDbContext>(options =>
        options.UseInMemoryDatabase("TradionIntegration"));
}
else
{
    var connectionString = builder.Configuration.GetConnectionString("DefaultConnection") ?? "Server=(localdb)\\mssqllocaldb;Database=Tradion;Trusted_Connection=True;MultipleActiveResultSets=true";
    builder.Services.AddDbContext<ApplicationDbContext>(options =>
...
.AddJwtBearer(options =>
{
    ...
    options.TokenValidationParameters = new TokenValidationParameters
    {
        ...
        ValidIssuer = builder.Configuration["Jwt:Issuer"] ?? "Tradion.Api",
        ValidAudience = builder.Configuration["Jwt:Audience"] ?? "Tradion.Web",
...
if (!builder.Environment.IsEnvironment("Testing"))
{
    builder.Services.AddHostedService<Tradion.Api.Services.PermitExpiryNotificationHostedService>();
    builder.Services.AddHostedService<Tradion.Api.Services.PaymentReminderHostedService>();
    builder.Services.AddHostedService<Tradion.Api.Services.OperationalRiskAlertsHostedService>();
}
