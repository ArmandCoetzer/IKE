...
export interface BuildSupplierQuotePartDraftsRequest {
  items: { partId: string; requestedQuantity?: number }[];
}
...
export class SupplierQuoteRequestsService {
  ...
  buildPartEmailDrafts(body: BuildSupplierQuotePartDraftsRequest): Observable<SupplierQuoteEmailDraftsResponse> {
    return this.http.post<SupplierQuoteEmailDraftsResponse>(`${API}/part-email-drafts`, body);
  }
