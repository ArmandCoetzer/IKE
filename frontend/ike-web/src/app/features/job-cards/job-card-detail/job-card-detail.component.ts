...
export class JobCardDetailComponent implements OnInit, OnDestroy {
  ...
  get visibleDocumentCount(): number {
    return this.preWorkDocuments.length + this.sitePhotoDocuments.length;
  }

  get waExpiredStandstill(): boolean {
    return !!this.job?.permitsRequired && !!this.job?.waExpiredStandstill;
  }
  ...
  get sortedPermits(): NonNullable<JobCardWorkDto['permits']> {
    if (!this.job?.permitsRequired) return [];
  ...
  private applyAccordionDefaults(): void {
    const j = this.job;
    if (!j) return;
    const docCount = this.visibleDocumentCount;
  ...
  canRequestSupplierStockForPlannedPart(p: PlannedPartDto): boolean {
    ...
    const linkedSupplierCount = (part?.supplierIds?.length ?? 0) + (part?.supplierId ? 1 : 0);
    return !!part && !part.isLabour && linkedSupplierCount > 0 && !!part.hasSupplierEmail;
  }
  ...
  plannedPartSupplierRequestBlockReason(p: PlannedPartDto): string {
    ...
    if (!part.supplierId && !(part.supplierIds?.length)) return 'No supplier linked';
  ...
  plannedPartSupplierRequestQueryParams(p: PlannedPartDto): Record<string, string | number> {
    ...
    return {
      partId: p.partId,
      supplierId: part?.supplierId ?? part?.supplierIds?.[0] ?? '',
  ...
  get workflowSteps(): {
    ...
    if (j.permitsRequired) {
      const permitsDone = this.hasApprovedPermit;
  ...
  get keyTimestamps(): { label: string; value?: string }[] {
    ...
    for (const d of this.job.documents ?? []) {
      ...
      if ((d.documentType || '').toLowerCase() === FINAL_CLIENT_SIGN_OFF_DOCUMENT_TYPE.toLowerCase()) continue;
  ...
  assignTechnician(): void {
    ...
    this.assigning = true;
    this.assignUserId = '';
    const asPermitManager = !!this.job?.permitsRequired && this.assignAsPermitManager;
